...
func AbortmultipartreadsetuploadHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/sequencestore/%s/upload/%s/abort", cfg.BaseURL, sequenceStoreId, uploadId)
		...
		var result models.AbortMultipartReadSetUploadResponse
...
func CreateAbortmultipartreadsetuploadTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_sequencestore_sequenceStoreId_upload_uploadId_abort",
		mcp.WithDescription(" Stops a multipart upload. "),
	...
	return models.Tool{
		Definition: tool,
		Handler:    AbortmultipartreadsetuploadHandler(cfg),
	}
}
