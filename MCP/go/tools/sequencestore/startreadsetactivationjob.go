...
func StartreadsetactivationjobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/sequencestore/%s/activationjob", cfg.BaseURL, sequenceStoreId)
		...
		var result models.StartReadSetActivationJobResponse
...
func CreateStartreadsetactivationjobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_sequencestore_sequenceStoreId_activationjob",
		mcp.WithDescription("Activates an archived read set. To reduce storage charges, Amazon Omics archives unused read sets after 30 days."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    StartreadsetactivationjobHandler(cfg),
	}
}
