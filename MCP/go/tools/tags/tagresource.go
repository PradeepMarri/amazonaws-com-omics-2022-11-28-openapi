...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_tags_resourceArn",
		mcp.WithDescription("Tags a resource."),
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description("The resource's ARN.")),
		mcp.WithObject("tags", mcp.Required(), mcp.Description("Input parameter: Tags for the resource.")),
	)
